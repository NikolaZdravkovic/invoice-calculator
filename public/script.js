...
fetch('https://nikolazdravkovic.github.io/invoice-calculator/api/invoice.json')
    ...
    const updateDiscountTotal = () => {
        discountTotal = 0;
        const discountAmountInputs = document.querySelectorAll('.discount-amount');
    ...
    data.forEach(item => {
        ...
        const discountedAmountInput = row.querySelector('.discounted-amount');
    ...
    newRow.innerHTML = `
                ...
                `;
    tbody === null || tbody === void 0 ? void 0 : tbody.appendChild(newRow);
    ...
    discountPercentAllInput.addEventListener('input', () => {
        ...
        data.forEach(item => {
            const row = tbody === null || tbody === void 0 ? void 0 : tbody.querySelector(`[data-id="${item.id}"]`);
            const price = item.price;
            const discountAmount = (price * discountPercentAll) / 100;
            const discountedAmount = price - discountAmount;
            const discountAmountInput = row === null || row === void 0 ? void 0 : row.querySelector('.discount-amount');
            const discountedAmountInput = row === null || row === void 0 ? void 0 : row.querySelector('.discounted-amount');
            discountAmountInput.value = !isNaN(discountAmount) ? discountAmount.toFixed(2) : '';
    ...
    // Add event listener to Discount in € field in new row
    const discountAmountAllInput = document.querySelector('.discount-amount-all');
    discountAmountAllInput.addEventListener('input', () => {
        ...
        data.forEach(item => {
            ...
            const discountPercentInput = row === null || row === void 0 ? void 0 : row.querySelector('.discount-percent');
            const discountedAmountInput = row === null || row === void 0 ? void 0 : row.querySelector('.discounted-amount');
            discountPercentInput.value = !isNaN(discountInPercentage) ? discountInPercentage.toFixed(2) : '';
            discountedAmountInput.value = !isNaN(discountedAmount) ? discountedAmount.toFixed(2) : '';
        });
    ...
    const discountedAmountAllInput = document.querySelector('.discounted-amount-all');
